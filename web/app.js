...
const COMMANDERS = {
  ...
  defensive: {
    ...
    emblem: "🛡️",
    tagline: "Prioriza Guardião, Arqueiro e Lanceiro; tenta acrescentar suporte se sobrarem recursos.",
...
`);
    ...
    replayModel = BattleReplay.create(catalog);
...
let replayModel;
function initialState() { return replayModel.initialState(); }
function applyEvent(state, event) { return replayModel.applyEvent(state, event); }
function buildState(events, count, snapshots = []) {
  return replayModel.buildState(events, count, snapshots);
}
...
function makeUnitNode(unit) {
  ...
  node.appendChild(el("div", "unit-cadence"));
  node.appendChild(el("div", "unit-fx"));
  return node;
}

function paintUnitNode(node, unit, round) {
  ...
  const cadence = node.querySelector(".unit-cadence");
  cadence.textContent = unit.reload && unit.readyRound > round
    ? `⟳ Arma pronta na R${unit.readyRound}` : "";
  cadence.hidden = !cadence.textContent;
  ...
  [1, 2].forEach((owner) => {
    ["front", "back"].forEach((lane) => {
      ...
      wanted.forEach((unit) => {
        ...
        paintUnitNode(node, unit, state.round);
...
function describe(event) {
  ...
  switch (event.type) {
    ...
    case "unit_moved":
      return `${actor} avançou para a linha de ${LANE_PT[event.metadata.lane] || event.metadata.lane}`;
    case "reloading":
      return `${actor} está recarregando; a arma volta em ${event.amount} rodada(s)`;
    case "out_of_range":
...
function buildManual() {
  ...
  catalog.units.forEach((unit) => {
    ...
    tags.appendChild(el("span", "tag", `Alcance ${unit.range} fileira(s) a partir da própria posição`));
    tags.appendChild(el("span", "tag", unit.reload ? `Recarga: ${unit.reload} rodada` : "Sem recarga"));
